...
public class BaseRequestData {
    ...
    public BaseRequestData(String _requestURL){
        super();
    ...
    public String getParamsString(){
        ...
        if(this.requestParams != null){
            for (String key : this.requestParams.keySet()) {
                ...
                if(returnStringBuffer.length() == 0 && (this.requestURL != null && !this.requestURL.contains("?"))){
                    returnStringBuffer.append("?");
                }else{
                    returnStringBuffer.append("&");
                }
                try {
                    value = URLEncoder.encode(Tools.string().nullStrToEmpty(value), "utf-8");
                }catch (Exception ex){
