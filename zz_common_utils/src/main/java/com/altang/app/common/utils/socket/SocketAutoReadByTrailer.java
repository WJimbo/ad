package com.altang.app.common.utils.socket;
 ...
 * @date 2019-02-20
 */
public class SocketAutoReadByTrailer {
    private SocketClient socketClient;
    public SocketAutoReadByTrailer(String ip, String port, int timeOutMill){
        socketClient = new SocketClient(new SocketClientAddress(ip,port,timeOutMill));
        /**
         * 设置读取策略为自动读取到指定的包尾
         */
        ...
        /**
         自动按包尾分割信息读取数据的发送配置
         */
        __i__setupReadToTrailerForSender(socketClient);
        /**
         自动按包尾分割信息读取数据的接收配置
         */
        __i__setupReadToTrailerForReceiver(socketClient);
...
// 对应removeSocketClientSendingDelegate
        socketClient.registerSocketClientSendingDelegate(new SocketClientSendingDelegate() {
            ...
            public void onSendPacketEnd(SocketClient client, SocketPacket packet) {
                client.readDataToData(ENDBYTES,false);
            }
        });
    }
...
// 对应removeSocketClientReceiveDelegate
    ...
    public void customSetHeartbeat(){
        /**
         * 设置自动发送的心跳包信息
         * 此信息动态生成
         *
    ...
    public static final byte[] ENDBYTES = new byte[]{21,22};
    ...
    private void __i__setupReadToTrailerForSender(SocketClient socketClient) {
         ...
         * 用于分隔多条消息
         */
        socketClient.getSocketPacketHelper().setSendTrailerData(ENDBYTES);
         ...
         */
//        socketClient.getSocketPacketHelper().setSendSegmentLength(0); // 设置发送分段长度，单位byte
//        socketClient.getSocketPacketHelper().setSendSegmentEnabled(true); // 设置允许使用分段发送，此值默认为false
    ...
    private void __i__setupReadToTrailerForReceiver(SocketClient socketClient) {
         ...
         * 用于分隔多条消息
         */
        socketClient.getSocketPacketHelper().setReceiveTrailerData(ENDBYTES);
